...
$page['title'] = _('Telephony');
$page['file'] = 'telephony.php';
$page['scripts'] = array();
$page['hist_arg'] = array();
...
//будет обработкой запроса аяксового дерева
if (isset($_REQUEST['serviceid']) && isset($_REQUEST['givememore'])) {
	$periods = array(
		'today' => _('Today'),
		'week' => _('This week'),
		'month' => _('This month'),
	...
	$srv_deps = $srvs['dependencies'];
	$srv_ids = array();
	foreach ($srv_deps as $dependence) {
		$srv_ids[] = $dependence['serviceid'];
	}
	$ch_services = API::Service()->get(array(
		'serviceids' => $srv_ids,
	...
	$triggers = zbx_objectValues($ch_services, 'trigger');
	$triggers = CTriggerHelper::batchExpandDescription($triggers);
	foreach ($ch_services as &$ch_service) {
		if ($ch_service['trigger']) {
			$ch_service['trigger'] = $triggers[$ch_service['trigger']['triggerid']];
		}
	}
	unset($ch_service);
...
//
	...
	if ($tree) {
		...
		$srv_wdgt->addPageHeader(_('Telephony'), get_icon('fullscreen', array('fullscreen' => $_REQUEST['fullscreen'])));
		$srv_wdgt->addHeader(_('Telephony'), $r_form);
		$srv_wdgt->addItem(BR());
